...
data Dict' : Nat -> (k : Type) -> Ord k -> Type -> Type where
  Empty : Dict' 0 k o v
  Node : (key : k)
      -> (val : v)
      -> (l : Dict' n k o v)
...
Dict : Nat -> (k : Type) -> {default %instance o : Ord k} -> Type -> Type
Dict n k {o = o} v = Dict' n k o v

-- --------------------------------------------------------------- [ Rotations ]
data InsertRes : Nat -> (k : Type) -> (o : Ord k) -> Type -> Type where
  Same : Dict' n k o v     -> InsertRes n k o v
  Grew : Dict' (S n) k o v -> InsertRes n k o v

%name InsertRes res, r

rotLeft : k -> v -> Dict' n k o v -> Dict' (S (S n)) k o v -> InsertRes (S (S n)) k o v
...
rotRight : k -> v -> Dict' (S (S n)) k o v -> Dict' n k o v -> InsertRes (S (S n)) k o v
...
-- --------------------------------------------------------------- [ Insertion ]
insert : (o : Ord k) => k -> v -> (t : Dict' n k o v) -> InsertRes n k o v
...
partial
update : Ord k => k -> (v -> v) -> Dict h k v -> (Dict h k v)
update x ufunc d = case updateValueBy (compare x) ufunc d of
  Same x => x

-- -------------------------------------------------------------------- [ List ]

fromList : (o : Ord k) => List (k, v) -> (n : Nat ** Dict' n k o v)
fromList [] = (0 ** Empty)
fromList ((k, v) :: xs) with (insert k v (getProof (fromList xs)))
  fromList ((k, v) :: xs) | (Same x) = (_ ** x)
  fromList ((k, v) :: xs) | (Grew x) = (_ ** x)

toList : Dict' n k o v -> List (k, v)
toList Empty = []
...
lookupUsing : Ord k => (k -> Ordering) -> Dict h k v -> Maybe v
...
lookup : Ord k => k -> Dict h k v -> Maybe v
lookup k d = lookupUsing (compare k) d

keys : Ord k => Dict h k v -> List k
keys Empty = Nil
keys (Node k _ l r b) = keys l ++ [k] ++ keys r

values : Ord k => Dict h k v -> List v
values Empty = Nil
values (Node _ v l r b) = values l ++ [v] ++ values r

length : Ord k => Dict h k v -> Nat
length Empty = Z
length (Node _ _ l r b) = 1 + length l + length r

hasKeyUsing : (Ord k) => (k -> Ordering) -> Dict h k v -> Bool
...
hasKey : (Ord k) => k -> Dict h k v -> Bool
hasKey k d = hasKeyUsing (compare k) d

hasValueUsing : Ord k => (v -> Bool) -> Dict h k v -> Bool
hasValueUsing f Empty = False
hasValueUsing f (Node k v l r b) =
  f v || (hasValueUsing f l) || (hasValueUsing f r)

hasValue : (Ord k, Eq v) => v -> Dict h k v -> Bool
hasValue v d = hasValueUsing (\x => v == x) d

getKeyUsing : Ord k => (v -> Bool) -> Dict h k v -> Maybe k
getKeyUsing f Empty = Nothing
getKeyUsing f d     =
    foldr (\acc,res => isRes f acc res) Nothing $ Dict.toList d
  where
...
getKey : (Ord k, Eq v) => v -> Dict h k v -> Maybe k
